...
void Object::loadModel(const std::string& path) {
    Assimp::Importer importer; // read file using assimp
...
Mesh Object::processMesh(aiMesh* mesh, const aiScene* scene) {
    ...
    for (unsigned int i = 0; i < mesh->mNumVertices; i++) {
        Vertex vertex;
        glm::vec3 vector; // temporary vector variable for reading from assimp vectors
    ...
    // walk through each mesh face and retrieve the corresponding vertex indices
...
unsigned int TextureFromFile(const char* path, const std::string& directory, bool gamma) {
    ...
    if (!data || nrComponents == 2 || nrComponents > 4) {
